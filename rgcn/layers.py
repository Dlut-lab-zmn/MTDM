...
class  ResUnionRGCNLayer(nn.Module):
    ...
    def propagate(self, g):
        g.update_all(lambda x: self.msg_func(x), fn.sum(msg='msg', out='h'), self.apply_func2)
    ...
    def forward(self, g, prev_h, emb_rel):
        ...
        if len(prev_h) != 0 and self.skip_connect: 
            ...
            if self.self_loop:
                node_repr =  node_repr2 + node_repr3 + loop_message
    ...
    def msg_func(self, edges):
        ...
        msg = torch.mm(ob_node, self.weight_neighbor) + torch.mm(relation, self.weight_neighbor2) + ob_node  +  relation 
