...
def test(model, history_list, test_list, num_rels, num_nodes, use_cuda, all_ans_list, all_ans_r_list, model_name, static_graph,global_graph,Formal, mode):
    """
    ...
    for time_idx, test_snap in enumerate(tqdm(test_list)):
        ...
        test_triples, final_score, final_r_score = model.predict(history_glist, num_rels, static_graph, global_graph, test_triples_input,use_cuda,Formal)
        ...
        for i in range(len(index)):
            if int(test_triples[i,1]) < len(id2relation):
                result_maitain.write(str(float(prop[i,0])) + '\t' + str(float(prop[i,1])) + '\t' + str(float(prop[i,2])) + '\t' + str(float(prop[i,3])) + '\t'+ str(float(prop[i,4])) + '\n')
                result_maitain.write(str(id2entity[int(index[i,0])]) + '\t' + str(id2entity[int(index[i,1])]) + '\t' + str(id2entity[int(index[i,2])]) + '\t' + str(id2entity[int(index[i,3])]) + '\t'+ str(id2entity[int(index[i,4])])+'\n')
                result_maitain.write(str(id2entity[int(test_triples[i,0])])+ '\t' + str(id2relation[int(test_triples[i,1])]) + '\t' + str(id2entity[int(test_triples[i,2])]) + '\t' +  str(time_idx) + '\n')
    ...
    if args.test and os.path.exists(model_state_file):
        ...
        for epoch in range(args.n_epochs):
            ...
            for train_sample_num in tqdm(idx):
                ...
                if args.add_entity_dissolution:
                    if adv is not None:
                        adv = [torch.from_numpy(_).long().cuda() for _ in adv] if use_cuda else [torch.from_numpy(_).long() for _ in adv]
                        adv = adv[0]
                if time_weight < 0.5: # transient
                    Formal = [True,False]
                else:
                    Formal = [True,True]
                loss_e, loss_r, loss_static,loss_parm,_ = model.get_loss(history_glist,output[0],adv, static_graph,global_graph, use_cuda, Formal)
            ...
            if epoch and epoch % args.evaluate_every == 0:
                ...
                if not args.relation_evaluation:  # entity prediction evalution
                    if mrr_filter < best_mrr:
                        ...
                        best_mrr = mrr_filter
                        torch.save({'state_dict': model.state_dict(), 'epoch': epoch,'Formal': Formal}, model_state_file)
                else:
